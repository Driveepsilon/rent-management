  ...
  const loadDashboardData = async () => {
    try {
      ...
      const [tenantsRes, propertiesRes, invoicesRes, paymentsRes] = await Promise.all([
        window.ezsite.apis.tablePage('tenants', { PageNo: 1, PageSize: 1000, Filters: [{ name: 'user_id', op: 'Equal', value: userData.ID }] }),
        window.ezsite.apis.tablePage('properties', { PageNo: 1, PageSize: 1000, Filters: [{ name: 'user_id', op: 'Equal', value: userData.ID }] }),
        window.ezsite.apis.tablePage('invoices', { PageNo: 1, PageSize: 1000, Filters: [{ name: 'user_id', op: 'Equal', value: userData.ID }] }),
        window.ezsite.apis.tablePage('payments', { PageNo: 1, PageSize: 1000, Filters: [{ name: 'user_id', op: 'Equal', value: userData.ID }] })
      ]);
